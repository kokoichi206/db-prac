...
# CREATE, INSERT 等の DDL, DML の情報も記録する。
log_statement = 'all'
```
...
### sec 10
高可用化とはサーバが故障しても別のサーバへと速やかに引き継ぎ運用を継続できること。

#### サーバの役割と呼び名
データの更新ができるサーバを「読み書きサーバ」または「マスタ」「プライマリ」と呼ぶ。プライマリ側のデータ変更を追跡するサーバを「スタンバイ」「スレーブ」と呼ぶ。
プライマリに昇格するまでクライアントから接続できないスタンバイを「ウォームスタンバイ」、クライアントから接続できて読み取り処理のみできるスタンバイを「ホットスタンバイ」と呼ぶ！

ウォームスタンバイもしくはホットスタンバイで、レプリケーションを用意しておく。

レプリケーションの方式にも色々と存在する

#### ストリーミングレプリケーション
PostgreSQL 本体に備わったレプリケーション機能。WAL をファイル単位ではなく、変更内容単位で送り、粒度の細かいレプリケーションが可能（ストリーミング）。

WAL: データベースの性能を担保しつつ、データの永続性を保証するための仕組み。


### sec 11

#### サーバーの死活監視
サーバの死活監視は、一般的に ping コマンドを使用する。

``` sh
ping -c 1 192.168.0.2
```

#### PostgreSQL の死活監視（プロセスの確認）
ps コマンドの確認でもいいが、本当に意図したプロセスを取れいてるか注意が必要。

9.3 以降のバージョンでは、`pg_isready`コマンドが使用できる
...
# e.g.
$ pg_isready -h 192.168.106.100
# これらのコマンドはクライアントなど別のマシンからでも実行可能！
```
...
不要領域を溜め込んだままにしないための処理が VACUUM で、「不要領域の再利用」と「トランザクション ID 周回問題の回避」が主な役割。

VACUUM と VACUUM FULL は別物。


### sec 14
インデックスへのアクセス性能が低下する場合の原因として、インデックスファイルの「肥大化」「断片化」「クラスタ性の欠落」が考えられる。

#### インデックスファイルの断片化
B-tree 固有の事象。
テーブルにデータを挿入していくと、インデックスには該当列のデータがリーフページに挿入されていく。インデックスのあるページがいっぱいになると、左右２つのページへと 50%-50% で分割される。

インデックスファイルの断片化が起こると、キャッシュヒット効率が悪化し、性能に影響を与える。

肥大化に関しては、基本は自動 VACUUM に任せても良い。それでも全てのインデックスが綺麗に再利用できるとは限らず、断片化の影響を改善するには `REINDEX` を行う。
...
REINDEX を実行中は、対象テーブルはロックされるため更新処理ができない。

#### クラスタ性の欠落
クラスタ性とは、テーブルデータの物理的な配置順序のこと。テーブルデータの並び順は、インデックスの並び順に近いとより効果的に検索を行える。

クラスタ性が欠落すると、インデックススキャンを行なっても、必要なデータの取得に複数のページを参照しなければならず、I/O の発行回数が増加し、性能に影響する。

テーブルのクラスタ性を確認するには、pg_stats ビューの correlation を参照する。
-1, 降順
0, ランダム
1, 昇順

``` sql
SELECT tablename, attname, correlation FROM pg_stats WHERE tablename = 'users';
```

クラスタ性を復活させるには、CLUSTER を行う。
...
``` sql
-- EXPLAIN の強い版
EXPLAIN (ANALYZE on, BUFFERS on) SELECT * FROM users WHERE id = 3;
-- 比較
...
## part 4
チューニングにかかる期間を見積もれますか？

### sec 15
問い合わせ（クエリ）を発行すると、RDBMS のプランなはさまざまな検索方法の中から処理コストが最小化になる選択や組み合わせを計算する。この最小化された検索方法のことを「実行計画」という。

#### 実行計画が最適化しない原因
テーブルやインデックスの行数/サイズなどの統計情報と、ページ/行単位の演算にかかるコスト値を用いて、実行計画を作成している。
統計情報の取得は一般にコストのかかる処理であり、定期的に実行しているため、クエリ発行タイミングによってはずれが生じる。

また、ハードウェア等の PostgreSQL 以外が原因となるケースもある

#### PostgreSQL が原因の場合
- コスト基準値の設定
    - PostgreSQL は、ハードディスク（HDD）へのランダムアクセスが最も処理コストが大きいと仮定している
    - random_page_cost, seq_page_cost
    - デフォルトでは HDD の想定だが、必要に応じて SSD 用に変更する
- 統計情報の取得頻度
- 統計情報の取得粒度
    - 取得する統計情報の数は、デフォでは 3 万件に固定
    - それ以上はあんまり実行計画の作成に影響しないってことかな？

#### PostgreSQL 以外が原因の場合
- ディスク性能によるもの
- ネットワーク性能によるもの
    - コネクションプール機能を持つ外部プログラムを利用し、接続のオーバヘッドを削減
- アプリケーションによるもの
    - PREPARE 文（プリペアドステートメント）
        - 実行計画をメモリ上に保存する機能
- バッチ処理や瞬間的な大量アクセスによるもの

#### ANALYZE オプション
実際にクエリを発行し、実行計画と実処理時間の両方を取得するオプション。
INSERT, DELETE などは実際に行ってしまうので注意。

また、**EXPLAIN 文が出力する実行計画コストには、通信コストやクライアント端末の表示にかかる時間は含まれていない**。特に SELECT 文で大量の行を出力する場合は、システム負荷は無視できないほど大きくなるが、EXPLAIN ではこれを考慮しない。そうよね、、、
